...
im = np.load('codeOutputs/TrainTestValid/2D3D/NormDist5000/5000TTV.npy', allow_pickle = True)
hc = np.load('codeOutputs/TrainTestValid/2D3D/NormDist5000/5000HC.npy', allow_pickle = True)
...
XPix, YPix = int(np.sqrt(aaaiTrainImages.shape[1])), int(np.sqrt(aaaiTrainImages.shape[1]))
...
TrainDataGen = ImageDataGenerator(rescale= None, horizontal_flip=False)
TrainDataGen.fit(aaaiTrainImages, augment=None)
TrainGenerator = TrainDataGen.flow(aaaiTrainImages, afTrainHC, batch_size=batch_size, seed=seed)
...
Tuner = Hyperband(ModelCreation, objective= 'val_loss', max_epochs =epochs, executions_per_trial =1, seed=seed, directory=os.path.normpath('codeOutputs/TrainTestValid/2D3D/TransportPropLog'), project_name='2DHyperparameters')

Tuner.search(TrainGenerator, verbose=2, validation_data= (aaaiValidationImages, afValidationHC), epochs=epochs, callbacks=[EarlyStop])
...
BestRegressor = Model.fit(TrainGenerator, validation_data = (aaaiValidationImages, afValidationHC), epochs=Epochs, callbacks =[EarlyStop], verbose=2)
...
plt.savefig('codeOutputs/TrainTestValid/2D3D/codeOutputs5000/2D_GridImages_5000_1000_1000.png',  dpi = 300)



afPredictedHC = Model.predict(aaaiTestImages)

np.savetxt('codeOutputs/TrainTestValid/2D3D/codeOutputs5000/2D_PredictedLogHC_GridImages_5000_1000_1000.csv', afPredictedHC)
save_model(Model, filepath= 'codeOutputs/TrainTestValid/2D3D/codeOutputs5000/Models/HCEstimation/2D_CNNModelForLogHCEstimation300Epochs', include_optimizer=True)
